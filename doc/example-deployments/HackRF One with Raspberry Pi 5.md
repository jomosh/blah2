...
- The above is likely to fail when you have run ```sudo docker compose up``` because the default Dockerfile was written for x64 machines. You need to replace it with [this file](/30hours/blah2/blob/main/contrib/raspberrypi/Dockerfile) that was written for ARM devices.
